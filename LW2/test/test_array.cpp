...
TEST(index_operator, test_simple_array)
{
    ...
    ASSERT_TRUE(array[0] == str[0]);
    ASSERT_TRUE(array[-1] == str[0]);
...
TEST(copy_constructor, test)
{
    ...
    ASSERT_TRUE(other_array.size == array.size);

    for (int i = 0; i < array.size; i++)
        ASSERT_TRUE(array[i] == other_array[i]);
}
...
TEST(equal_operator, test)
{
    ...
    ASSERT_TRUE(first_array.size == sec_array.size);
    for (int i = 0; i < first_array.size; i++)
        ASSERT_TRUE(first_array[i] == sec_array[i]);
}
...
TEST(move_equal_operator, test)
{
    ...
    for (int i = 0; i < 6; i++)
        ASSERT_TRUE(first_array[i] == sec_str[i]);
}
